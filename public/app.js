...
async function openPlayer(url) {
    ...
    try {
        ...
        dom.currentTitle.textContent = data.title || 'Unknown Title';
        ...
        if (data.episodes.length > 0) {
            ...
            if(firstEpisode && firstEpisode.url) {
                playEpisode(firstEpisode.url, firstEpisode.episode_id || '1');
            } else {
                playEpisode(data.episodes[0], '1');
            }
        } else {
        ...
        dom.epGrid.innerHTML = `
            <div style="grid-column:1/-1; text-align:center; padding:40px;">
                ...
                <p style="color:var(--text-muted); font-size:0.9rem; margin-top:10px;">${e.message}</p>
                <button class="nav-btn" onclick="retryLoadDrama()" style="margin-top:20px;">
                    <i class="fas fa-redo"></i> Retry
                </button>
                <button class="nav-btn" onclick="closePlayer()" style="margin-top:10px; margin-left:10px;">
...
function retryLoadDrama() {
    if(currentDramaUrl) {
        openPlayer(currentDramaUrl);
    } else {
        closePlayer();
    }
}
    ...
    episodes.forEach(ep => {
        ...
        btn.onclick = () => {
            if(typeof ep === 'object' && ep.url) {
                ...
                showAlert('Invalid episode data', 'error');
            }
        };
...
function playEpisode(videoUrl, epNum) {
    ...
    document.querySelectorAll('.ep-btn').forEach(b => {
        const btnNum = b.textContent;
        b.classList.toggle('active', btnNum === epNum);
    });
    ...
    if (playPromise !== undefined) {
        playPromise.then(() => {
            ...
            showAlert(`Loaded Episode ${epNum}. Click play to start.`);
        });
    }
}
